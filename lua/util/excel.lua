...
local function LoadNPIO()
    local xls = xlua.load_assembly("NPOI") -- xls
    local et = xls:GetExportedTypes()
    for i=0, et.Length - 1 do
    	print(i, et[i])
    end
    local xlsx = xlua.load_assembly("NPOI.OOXML") -- xlsx
    local et = xlsx:GetExportedTypes()
    for i=0, et.Length - 1 do
    	print(i, et[i])
    end
    print("xls", dump(xls))
    print("xlsx", dump(xlsx))
    as = CS.System.AppDomain.CurrentDomain:GetAssemblies(); 
    for i = 0, as.Length-1 do print(i, as[i])end
...
local function OpenExcel( path )
    ...
    if      path:sub(-5) == ".xlsx" then
        local XWorkbook = CS.NPOI.XSSF.UserModel.XSSFWorkbook
        wb = XWorkbook(path)
    elseif path:sub(-4) == ".xls" then
        local inStream = System.IO.FileStream(path, System.IO.FileMode.Open);
        local HWorkbook = CS.NPOI.HSSF.UserModel.HSSFWorkbook
