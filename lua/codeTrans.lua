...
local function TransOneV2( fpath, regular_jp, t )
    ...
    local fti = io.open("tmp-ins-sql/" .. fpath:gsub("/", "_") .. "-insert.sql", "w")
	...
	local fts = io.open("tmp-sel-sql/" .. fpath:gsub("/", "_") .. "-select.sql", "w")
	fts:write(sql)
	fts:close()
...
-- local parten   = '[^\0-\127]*\227[\129-\131][\128-\191][^\0-\127]*' -- 包含一个日文字母 \227[\128-\132][\128-\191]
local parten = '[\0-\127\194-\253][\128-\191]+[\0-\253]*' -- jpzh
...
-- local regular_jp  = [['[^'"\n]*[\u3040-\u3126]+[^'"\n]*']] 		-- c   'jp'
-- local regular_jp2 = [["[^'"\n]*[\u3040-\u3126]+[^'"\n]*"]] 	 	-- c   "jp"
-- local regular_xml = [[>[^-><'";]*[\u3040-\u3126]+[^-><'";]*<]]  -- xml >jp<
local regular_xml = '>([^><]*[\0-\127\194-\253][\128-\191][\128-\253][^><]+)<'  -- xml
...
util.GetFiles(
    {
        ...
        -- "/Users/cn/dark/client/dark-client/Assets/workspace/ui/help/ja",
		-- "/Users/cn/dark/client/dark-client/Assets/Scripts"
   ...
   , function ( fp )
    ...
    --    TransOneV2(fp, parten, 'c')
       TransOneV2(fp, regular_xml, 'xml')
   end
